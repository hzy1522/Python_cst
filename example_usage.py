...
def gan_demo(create_antenna_data):
    ...
    # 训练GAN
    forward_gan = True
    gan_history = system.train_gan(X_train, y_train, epochs=5000, batch_size=128, forward_gan=forward_gan)

    # 可视化GAN训练结果
    system.visualize_gan_results(gan_history)
    ...
    # 可视化生成结果
    system.visualize_gan_results(gan_history, generated_designs, generated_performances)
    ...
    if len(generated_designs) > 0:
        ...
        plot_s11_comparison_advanced(system, float(best_design[0]), float(best_design[1]),
...
def plot_s11_comparison_advanced(system, patch_length, patch_width, csv_file_path,
    ...
    # 1. 使用GAN模型预测S11曲线
    s11_curve, s11_min, freq_at_s11_min, far_field_gain = system.predict_s11_from_dimensions(
        system, patch_length, patch_width
    )
    ...
    try:
        ...
        if frequency_column is not None:
            ...
            # 使用系统默认频率点
            frequencies = system.freq_points if hasattr(system, 'freq_points') else np.linspace(2.0, 3.0,
                                                                                                len(actual_s11_data))
    ...
    # 绘制预测的S11曲线
    pred_line = plt.plot(system.freq_points, s11_curve,
